...
# Build reverse map: ticker → company name (for display)
...
def get_filtered_options(query):
    """
    Filter STOCK_SUGGESTIONS by query (matches company name or ticker).
    Returns list of display strings: "Tesla  (TSLA)"
    Also prepends a raw ticker option if query looks like a ticker.
    """
    if not query:
        return []
    ...
    # Always add a "search as typed" fallback at the bottom
    results.append(NO_MATCH_LABEL)

    return results[:8]  # max 7 suggestions + fallback


def label_to_ticker(label, raw_input):
    """Extract ticker from a display label like 'Tesla  (TSLA)'."""
    if label == NO_MATCH_LABEL:
        return raw_input.strip().upper()
    # Extract ticker from inside parentheses
...
# Live filtering selectbox — only shows when user has typed something
...
# ==================================================
# Section 2 — Forecast Button
# ==================================================
if not model_choice.startswith("ARIMA"):
...
# ==================================================
# Section 5 — Cross-check Result (Overlay)
# ==================================================
if st.session_state.crosscheck_done:
    ...
    # ---- Trend Mode ----
        ...
        print(f"[DEBUG] Predicted dirs (first 5): {predicted_dirs[:5]}")
        print(f"[DEBUG] Actual dirs    (first 5): {actual_dirs[:5]}")
